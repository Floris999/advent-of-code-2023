...
function extractAndCombineNumbers($inputText) {
	// splitst de tekst in regels op basis van de "end of line
    $lines = explode(PHP_EOL, $inputText);
    $result = 0;

    foreach ($lines as $line) {
    	// zoekt naar alle cijfers (0-9) in een tekst met behulp van een reguliere expressie
        preg_match_all('/\d/', $line, $matches);
        $numbers = $matches[0];
        
    // $first = $numbers[0];: Dit haalt het eerste cijfer uit de array $numbers.
		// $last = end($numbers);: Dit haalt het laatste cijfer uit de array $numbers door de arraypointer naar het laatste element te verplaatsen.
		// $result += intval($first . $last);: Dit voegt het getal samen door het eerste en het laatste cijfer aan elkaar te plakken en zet het resultaat om naar een integer. Dit wordt toegevoegd aan de totale som ($result).

        if (!empty($numbers)) {
            $first = $numbers[0];
            $last = end($numbers);
            $result += intval($first . $last);
        }
    }
...
$outputSum = extractAndCombineNumbers($inputText);
